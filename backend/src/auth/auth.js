...
passport.use(new LocalStrategy({
    ...
    async (email, password, callback) => {
        try {
            ...
            const saltBuffer = user.salt.buffer || user.salt; // Ajuste seguro
            ...
            crypto.pbkdf2(password, saltBuffer, 310000, 16, 'sha256', (err, hashedPassword) => {
                ...
                const userPasswordBuffer = user.password.buffer || user.password;

                if (!crypto.timingSafeEqual(Buffer.from(userPasswordBuffer), hashedPassword)) {
                    return callback(null, false);
                }
